...
public class CollisionSystem implements IProcess {
    ...
    public void process(GameData gameData, World world) {
        List<Object> listOfEventsToPost = new ArrayList<>();
        ...
        for(int i = 0; i < list.size(); i++) {
            for(int j = i + 1; j < list.size(); j++) {
                if(list.get(i).isCollidable() && list.get(j).isCollidable() && intersects(list.get(i), list.get(j))) {
                    ...
                    listOfEventsToPost.add(new CollisionEvent(list.get(i).getId(), list.get(j).getId()));
                    listOfEventsToPost.add(new CollisionEvent(list.get(j).getId(), list.get(i).getId()));
                }
            }
        }
        
        listOfEventsToPost.forEach(event -> Dispatcher.post(event, world));
    }
